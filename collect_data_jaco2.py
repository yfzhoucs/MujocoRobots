...
class Executor:
    def __init__(self, interface, start_angles, start_gripper_status, object_keyword='target'):
        ...
        # print(transformations.euler_from_quaternion(self.interface.get_orientation('EE'), "rxyz"))
        # exit()
...
def init_env(robot_config, interface):
    ...
    #     gripper.set_gripper_status(-0.5)
    #     u[-1] = gripper.get_gripper_status()
    #     return u
    ...
    if action_inst == 0:
        ...
        if target_name != 'bottle' and target_name != 'milk':
            ...
            e.append(MoveTo(interface, ctrlr, target_func_above_target_move_in, gripper_control_func_close, time_limit=30))
            ...
            e.append(MoveTo(interface, ctrlr, target_func_above_target_move_in_bottle, gripper_control_func_close, time_limit=30))
            ...
            e.append(MoveTo(interface, ctrlr, target_func_above_target_move_in_milk, gripper_control_func_close, time_limit=30))
        ...
        e.append(MoveTo(interface, ctrlr, target_func_place, gripper_control_func_close, error_limit=0.02))
        e.append(MoveTo(interface, ctrlr, target_func_place, gripper_control_func_open, time_limit=15))
        e.append(MoveTo(interface, ctrlr, target_func_release, gripper_control_func_open, error_limit=0.02))
    else:
...
def collect_data(data_dir, num):
    # Init Simulator
    robot_config = arm('jaco2_3.xml', folder='./my_models/jaco2_abr')
    interface = Mujoco(robot_config, dt=0.008, visualize=True, create_offscreen_rendercontext=False)
    interface.connect(joint_names=['joint0', 'joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint_thumb'], camera_id=-1)
